                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                     <tr v-for="product in products.data" :key="product.id">
                        <td>{{product.id}}</td>
                        <td>{{product.name}}</td>
                        <td><img :src="'data:image/png;base64,' + product.photo" width="100" alt="product"></td>
                        <td>{{product.price * 100}}</td>
                        <td>{{product.description | truncate(30, '...')}}</td>
                        <td>{{product.category.name}}</td>
                        <td>{{product.created_at|myDate}}</td>
                        <td>
                          <a v-if="$gate.isAdmin()" href="#" @click="editModal(product)">
                              <i class="fa fa-2x fa-edit blue"></i>  
                          </a>
                          &nbsp;
                          <a v-if="$gate.isAdmin()" href="#" @click="deleteProduct(product.id)">
                              <i class="fa fa-2x fa-trash red"></i>
                          </a> 
                          <a v-if="$gate.isUser()" href="#" @click="updateReport(product.id)">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                          <div class="form-group col-auto" v-if="editmode">
                            <img :src="'data:image/png;base64,' + form.photo" width="200px" alt="product">
                          </div>
                          <div class="form-group col-auto">
                              <label>Name</label>
                              <input v-model="form.name" type="text" name="name"
                                  class="form-control" :class="{ 'is-invalid': form.errors.has('name') }">
                              <input v-model="form.name" type="text" name="oldName"
        ...
        methods: {
          ...
          updateProduct(){
              ...
              .then((response) => {
                  ...
                  fetch("http://18.138.81.76:8000/api/v1/recognition/faces?subject="+ this.form.name +"&det_prob_threshold=0.9", requestAiOptions)
