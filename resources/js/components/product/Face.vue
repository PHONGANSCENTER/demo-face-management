        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Face List</h3>
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-hover">
                ...
                <tbody>
                  <tr v-for="(product, index) in faces.data" :key="product.id">
                    <td>{{ index + 1 }}</td>
                    <td>{{ product.name }}</td>
                    <td>
                      <img
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      ...
      axios
        .get("api/face?page=" + page)
        .then(({ data }) => (this.faces = data.data));

      this.$Progress.finish();
    },
    loadProducts() {
      // if(this.$gate.isAdmin()){
      axios.get("api/face").then(({ data }) => (this.faces = data.data));
      // }
    },
    loadCategories() {
      axios
