...
class ProductController extends BaseController
{
    ...
    public function index()
    {
        $user = Auth::User();
        if ($user->type == 'admin') {
            $products = $this->product->where('reported', 0)->latest()->with('category', 'tags')->paginate(10);
        } else {
            $products = $this->product->where([['name', $user->name],['reported', 1]])->latest()->with('category', 'tags')->paginate(10);
        }
